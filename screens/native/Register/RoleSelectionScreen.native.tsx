    ...
    return (
        <SafeAreaView className="flex-1 bg-white dark:bg-[#151718]">
            <View className="flex-1 px-6 pt-10">
                ...
                <View className="mt-10 space-y-6 px-4">
                    {roles.map((role, index) => {
                        ...
                        return (
                            <Animated.View
                                key={role.id}
                                entering={FadeInDown.delay(400 + index * 200).duration(600)}
                                className="w-full mb-8"
                            >
                                <TouchableOpacity
                                    onPress={() => setSelectedRoleId(role.id)}
                                    activeOpacity={0.8}
                                    className="w-full bg-gray-50 dark:bg-gray-800 rounded-[20px] items-center justify-center border-2 py-6"
                                    style={{
                                        borderColor: isSelected ? primaryColor : (isDark ? '#3a3a3a' : '#F3F4F6'),
                                        backgroundColor: isSelected ? (isDark ? '#1a1d1e' : '#FFFFFF') : (isDark ? '#1f1f1f' : '#F9FAFB'),
                                        shadowColor: '#000',
                ...
                <Animated.View entering={FadeInDown.delay(1000).duration(600)} className="mb-10">
                    <TouchableOpacity
